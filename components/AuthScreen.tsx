...
import { loginAction, signupAction, googleAction, type AuthState } from '@/lib/actions';
      ...
      <div className="auth-wrap" style={{ display: 'grid', gap: '2rem', width: '100%', maxWidth: 980, gridTemplateColumns: '1.1fr 1fr', alignItems: 'center' }}>
        ...
        <section className="auth-brand" style={{ display: 'flex', flexDirection: 'column', gap: '1.6rem', padding: '1rem' }}>
          ...
          <div style={{ position: 'relative', height: 200, display: 'grid', placeItems: 'center' }} aria-hidden>
            <span style={{ position: 'absolute', width: 20, height: 20, borderRadius: '50%', border: '1px solid var(--border-2)', animation: 'ping-ring 3s ease-out infinite' }} />
            <span style={{ position: 'absolute', width: 20, height: 20, borderRadius: '50%', border: '1px solid var(--border-2)', animation: 'ping-ring 3s ease-out infinite 1s' }} />
            <span style={{ position: 'absolute', width: 20, height: 20, borderRadius: '50%', border: '1px solid var(--border-2)', animation: 'ping-ring 3s ease-out infinite 2s' }} />
            <span className="live-dot" style={{ width: 12, height: 12 }} />
          </div>
          <div>
            <h1 style={{ fontSize: '2rem', fontWeight: 680, letterSpacing: '-0.03em', lineHeight: 1.1 }}>A dashboard that watches itself.</h1>
        ...
        <section className="glass" style={{ padding: '1.8rem' }}>
          ...
          <form action={formAction} style={{ display: 'flex', flexDirection: 'column', gap: '.8rem' }}>
            {mode === 'signup' && (
              <label style={labelStyle}>Name<input name="name" required className="input" placeholder="Ada Lovelace" autoComplete="name" /></label>
            )}
            ...
            {state?.error && <p role="alert" style={{ fontSize: '.8rem', color: 'var(--text)', background: 'color-mix(in oklab, var(--text) 8%, transparent)', padding: '.55rem .7rem', borderRadius: 10 }}>{state.error}</p>}

            <button type="submit" className="btn btn-primary" disabled={pending} style={{ marginTop: '.3rem', padding: '.7rem' }}>
              {pending ? 'One moment…' : mode === 'login' ? 'Sign in' : 'Create account'}
            </button>
          </form>

          {enableGoogle && (
            <form action={googleAction} style={{ marginTop: '.7rem' }}>
              <button type="submit" className="btn" style={{ width: '100%', padding: '.7rem' }}>Continue with Google</button>
            </form>
          )}
          ...
          {mode === 'login' && (
            <button
              type="button"
              onClick={() => { setEmail('demo@beacon.local'); setPassword('demo1234'); }}
              className="text-mute"
              style={{ marginTop: '1rem', fontSize: '.78rem', background: 'none', border: 'none', cursor: 'pointer', width: '100%', textAlign: 'center' }}
            >
